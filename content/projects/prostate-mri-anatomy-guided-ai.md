...
summary: >-
  This work presents a fully automated, anatomically guided deep learning
  pipeline for prostate cancer risk stratification from routine MRI,
  integrating an nnU-Net module for segmenting the prostate gland and its
  zones, a classification module based on the UMedPT Swin Transformer
  foundation model fine-tuned with anatomical priors and clinical data, and
  a VAE-GAN framework generating counterfactual heatmaps that localize
  decision-driving regions. Developed on 1,500 PI-CAI cases for
  segmentation and 617 biparametric MRIs from the CHAIMELEON challenge for
  classification, segmentation reached Dice scores of 0.92 to 0.95 across
  zones, and a three-scale classification ensemble with anatomical priors
  reached an AUC of 0.79, outperforming the 2024 CHAIMELEON winners. In a
  prospective multi-center in-silico trial with 20 clinicians, AI assistance
  raised diagnostic accuracy from 0.72 to 0.77, raised inter-rater agreement
  from 0.43 to 0.53, and cut review time by 40%, supporting the pipeline's
  potential as a virtual biopsy in clinical practice.
year: 2025
status: "completed"
tags:
  - foundation-models
