...
summary: >-
  This conference challenge compared machine-learning reconstruction models
  for gamma-aminobutyric acid (GABA)-edited MR spectroscopy using only
  one-quarter of the transients typically required for a complete scan,
  across three tracks: simulated data, in vivo data with matched acquisition
  parameters, and in vivo data with different parameters. Performance was
  scored on mean squared error, signal-to-noise ratio, linewidth, and a
  proposed shape-score metric, using an openly released baseline model and
  simulated data. Of three compared submissions, a covariance-matrix CNN
  performed best on simulated data, while a vision transformer on a
  spectrogram representation performed best on both in vivo tracks.
  Reconstructions from just 80 transients matched or beat conventional
  320-transient results on SNR, linewidth, and fit error, though some models
  optimized those metrics without truly improving spectral quality,
  pointing to a need for more robust evaluation metrics.
year: 2024
status: "completed"
tags:
  - mri
